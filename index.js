...
client.on(Events.InteractionCreate, async interaction => {
  ...
  if (interaction.commandName === 'register') {
    ...
    saveData(data);
    await interaction.reply({ content: `✅ 登録しました！\n勝率: ${winRate}\n勝: ${wins} 負: ${losses.toFixed(2)} 分: ${draws.toFixed(2)}\n合計: ${matches}`, ephemeral: true });
  }
    ...
    saveData(data);
    await interaction.reply({ content: `✅ 戦績を更新しました！\n\n📊 成績:\n勝: ${u.W} 負: ${u.L} 分: ${u.D} 合計: ${u.M}\n勝率: ${(u.P * 100).toFixed(2)}%\n\n🎯 ${goal1 * 100}%までに必要勝利数: ${needed1}\n🎯 ${goal2 * 100}%までに必要勝利数: ${needed2}`, ephemeral: true });
  }
    ...
    await interaction.reply({ content: `📊 あなたの戦績：\n勝: ${u.W} 負: ${u.L} 分: ${u.D} 合計: ${u.M}\n勝率: ${(u.P * 100).toFixed(2)}%`, ephemeral: true });
  }
    ...
    if (hour < 0 || hour > 23) {
      await interaction.reply({ content: '⚠️ 時間は 0～23 または -1（通知OFF）で指定してください。', ephemeral: true });
      return;
    }
    reminderData[userId] = { hour, channelId: channel.id };
    saveReminderData(reminderData);
    await interaction.reply({ content: `✅ 毎日 ${hour}:00 に ${channel.name} で戦績リマインダー通知を送信します！`, ephemeral: true });
  }
    ...
    rankReminderData[userId] = { hour, channelId: channel.id };
    saveRankReminderData(rankReminderData);
    await interaction.reply({ content: `✅ 毎日 ${hour}:00 に ${channel.name} でランクマ参加アンケートを送信します！`, ephemeral: true });
  }
});
