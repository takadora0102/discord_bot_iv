...
cron.schedule('* * * * *', async () => {
  ...
  for (const userId in reminderData) {
    const { hour: h, channelId } = reminderData[userId];
    const key = `${userId}-${h}`;
    if (h === hour && minute === 0 && !sentReminders.has(key)) {
  ...
  for (const userId in rankReminderData) {
    const { hour: h, channelId } = rankReminderData[userId];
    const key = `rank-${h}`;
    if (h === hour && minute === 0 && !sentReminders.has(key)) {
      sentReminders.add(key);
      try {
        ...
        const collector = message.createMessageComponentCollector({ time: 3 * 60 * 60 * 1000 });
        const result = { 参加: [], 不参加: [] };

        collector.on('collect', async i => {
          if (i.customId === 'rank参加') result.参加.push(i.user.username);
          if (i.customId === 'rank不参加') result.不参加.push(i.user.username);
          await i.reply({ content: '回答ありがとう！', ephemeral: true });
        });

        collector.on('end', async () => {
          await channel.send({
            content: `@everyone ランクマ参加集計結果：\n✅参加: ${result.参加.join(', ') || 'なし'}\n❌不参加: ${result.不参加.join(', ') || 'なし'}`,
            allowedMentions: { parse: ['everyone'] }
          });
        });
